...
describe('TypegooseCoreModule', () => {
  describe('forRoot', () => {
    it('should return module that provides a mongoose connection', () => {
      ...
      jest
        ...
        .mockReturnValue(connection as any);
  ...
  describe('forAsyncRoot', () => {
    ...
    describe('Connection Name', () => {
      ...
      it('creates the mongoose connection', () => {
        ...
        expect(DbConnectionToken.useFactory(optionsFromOptionFactory)).toBe(
          connection,
        );
        expect(mongoose.createConnection).toHaveBeenCalledWith(
          optionsFromOptionFactory.uri,
          {
            other: 'options',
            can: 'work',
          },
    ...
    describe('different types', () => {
      describe('useFactory', () => {
        ...
        it("injects the factory's async options into DEFAULT_DB_CONNECTION_NAME", () => {
          ...
          const typegooseModuleOptionsFactoryProvider = module.providers.find(
            (provider) =>
              (provider as FactoryProvider).provide ===
              TYPEGOOSE_MODULE_OPTIONS,
          ) as FactoryProvider;
          expect(typegooseModuleOptionsFactoryProvider.inject).toBe(
            wantedDependencies,
          );
          expect(typegooseModuleOptionsFactoryProvider.useFactory).toBe(
            mockOptionFactory,
          );
        });
      });
      ...
      describe('useClass', () => {
        ...
        it("creates a factory called TYPEGOOSE_MODULE_OPTIONS that calls TypegooseConfigService's createMongooseOptions", async () => {
          ...
          expect(typegooseModuleOptionsFactoryProvider.inject).toEqual([
            mockConfigClass,
          ]);
          // Then provides wrapper factory, which will get injected TypegooseConfigService
          await typegooseModuleOptionsFactoryProvider.useFactory(
            mockConfigClass,
          );
  ...
  describe.skip('Disconnect in onModuleDestroy', () => {
